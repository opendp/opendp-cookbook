The OpenDP Cookbook
===================

The OpenDP Cookbook is a collection of recipes for the OpenDP Library.
We check that the recipes run with the latest version of the library,
but they might not be the best solution for your particular problem.

In addition to the cookbook,
we suggestion consulting the `OpenDP Library documentation <https://docs.opendp.org/>`_,
and emailing `contact@opendp.org <mailto:contact@opendp.org>`_
for an invitation to the OpenDP Slack.


.. toctree::
   :maxdepth: 1
   
   basics/index
   theory/accuracy-pitfalls

